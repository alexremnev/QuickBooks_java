...
//    private List<SalesTransaction> salesTransactionList = new ArrayList<>();
//    private Map<String, BigDecimal> taxRateMap;
//
//    @Autowired
//    public ReportService(OauthService oauthService, ReportDAO reportDAO, TaxRateDAO taxRateDAO) {
//        this.oauthService = oauthService;
//        this.reportDAO = reportDAO;
//        this.taxRateDAO = taxRateDAO;
//    }
//
//    public void calculate() throws FMSException {
//        List<SalesTransaction> entities = oauthService.getDataService().findAll(new Invoice());
//        calculate(entities);
//    }
//
//    private List<SalesTransaction> calculate(List<SalesTransaction> entities) throws FMSException {
...
////        String query = select($(invoice)).where($(invoice.getDocNumber()).eq("1012")).generate();
////        query = query.replaceAll("tring.", "");
////        QueryResult result = dataService.executeQuery(query);
////        List<Invoice> entities = new ArrayList<>();
////        Invoice testEntity = (Invoice) result.getEntities().get(0);
////        PhysicalAddress physicalAddress = new PhysicalAddress();
////        physicalAddress.setCountrySubDivisionCode("CA");
////        testEntity.setShipAddr(physicalAddress);
////        entities.add(testEntity);
//
//        if (entities.size() == 0) return new ArrayList<>();
//        for (SalesTransaction entity : entities) {
//            setTaxCode(entity);
//            if ((entity.getShipAddr() == null) || (entity.getShipAddr().getCountrySubDivisionCode() == null)) continue;
...
//        salesTransactionList.add(new SalesReceiptService());
//        salesTransactionList.add(new Invoice());
//        salesTransactionList.add((new CreditMemo()));
//        DataService dataService = oauthService.getDataService();
//        for (SalesTransaction salesTransaction : salesTransactionList) {
//            List<SalesTransaction> entityList = dataService.findAll(salesTransaction);
//            save(entityList);
//        }
//    }
//
//    private void save(List<SalesTransaction> entities) throws FMSException {
//        for (SalesTransaction entity : entities) {
...
//        List<TaxRate> taxRates = taxRateDAO.list();
//        taxRateMap = new HashMap<>();
//        for (TaxRate taxRate : taxRates) taxRateMap.put(taxRate.getCountrySubDivisionCode(), taxRate.getTax());
//        return taxRateMap;
//    }
//
